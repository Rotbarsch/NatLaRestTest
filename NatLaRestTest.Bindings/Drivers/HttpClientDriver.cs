...
/// <summary>
/// Provides a shared <see cref="HttpClient"/> and tracks the current <see cref="HttpResponseMessage"/> for a scenario.
/// Supports configuring base address, default timeout, request headers and SSL certificate validation.
/// </summary>
public class HttpClientDriver : IHttpClientDriver, IDisposable
{
    /// <summary>
    ...
    /// <summary>
    /// Gets or sets the current HTTP response captured by the driver.
    /// </summary>
    ...
    public async Task SendRequest(string httpMethod, string relativePath, string? requestBody = null, string contentType = "application/json")
    {
    ...
    public async Task SaveResponseStreamToFile(string filePath)
    {
        ...
        _loggingDriver.WriteLine($"Wrote '{fileStream.Length}' bytes to '{Path.GetFullPath(filePath)}'.");
    }
    ...
    {
        AssertResponseAvailable();
        Assert.IsTrue(CurrentResponse!.Headers.TryGetValues(headerName, out var headerValues), $"No header with key '{headerName}' in current response.");
    ...
    private async Task LogHttpResponse(HttpResponseMessage currentResponse)
    {
        ...
        sb.AppendLine($"Received response {currentResponse.StatusCode} with body size of {bodyLength} bytes with headers:");
